...
public interface IAhpClient : IAsyncDisposable
{
    ...
    /// <summary>Begins an authentication attempt using a client-brokered flow advertised by the host.</summary>
    Task<AuthBeginResult> AuthBeginAsync(AuthBeginParams parameters, CancellationToken cancellationToken = default);

    /// <summary>Submits a token, preserving any attempt/account binding without a legacy fallback.</summary>
    Task<AuthenticateResult> AuthenticateAsync(AuthenticateParams parameters, CancellationToken cancellationToken = default);
    ...
    /// <summary>Subscribes to the host's accounts channel when authentication support was advertised.</summary>
    Task<(SubscribeResult Result, Subscription Sub)> SubscribeAccountsAsync(SubscriptionDeliveryOptions? delivery = null, CancellationToken cancellationToken = default);
