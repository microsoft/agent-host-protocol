...
Every push-style interaction in AHP lives on a **channel** — a URI-identified subscribable resource. The root catalogue (`ahp-root://`), each session (`ahp-session:/<uuid>`), each chat (`ahp-chat:/<uuid>`), each terminal (`ahp-terminal:/<id>`), each changeset (`ahp-changeset:/<id>`), and each canvas (`ahp-canvas:/<id>`) is its own channel. A channel MAY hold an immutable state tree, or it MAY be a stateless pub/sub topic (planned: logging, MCP relay, LSP relay).
...
| Concept | Description |
|---|---|
| **Channels** | URI-identified subscribable resources. State channels (root, sessions, chats, terminals, changesets, canvases) hold an immutable state tree; stateless channels exist for streaming data. Every message carries `channel: URI` for routing. |
| **State** | Immutable tree per state channel — root state, per-session state, per-chat state, per-terminal state, per-changeset state, per-canvas state. |
